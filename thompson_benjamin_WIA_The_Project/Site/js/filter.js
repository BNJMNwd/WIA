...
$(document).ready(function(){
    ...
    window.chartHeight = Number($('.chart-area-filter').height());
    window.barWidth = $('.chart-area-filter .chart-bar-filter').width();
    window.highestYlabel = Number($('.chart-y-axis_filter p').first().html());
    window.chartHeightArea = window.chartHeight - Number($('p.axis_filter-value').first().height());
    window.chartScale = chartHeightArea / window.highestYlabel;
    window.barSpacing = Number($('.chart-area-filter').attr('bar-spacing'));
    positionBars();

});

function positionBars(){
    $('.chart-area-filter .chart-bar-filter').each(function(index){
        ...
        $('.chart-x-axis_filter').append('<p style="left:'+(barPosition-(window.barWidth/2))+'px;">'+$(this).attr('label')+'</p>');
